...
def custom_get_num_layer_for_swin(var_name, num_max_layer, depths):
    if var_name.startswith("img_backbone.patch_embed"):
        return 0
    elif "level_embeds" in var_name or "camera_embeds" in var_name or "query_embedding" in var_name \
        or "input_proj" in var_name or "position_encoder" in var_name or "fpe" in var_name \
        or "time_embedding" in var_name or "ego_trans_embedding" in var_name \
        or "temporal_bev_proj" in var_name or "temporal_pos_embedding" in var_name:
        return 0
...
def get_vit_lr_decay_rate(var_name, num_layers=12):
    """
    ...
    if var_name.startswith("img_backbone"):
        if ".pos_embed" in var_name or ".patch_embed" in var_name:
            layer_id = 0
        elif "level_embeds" in var_name or "camera_embeds" in var_name or "query_embedding" in var_name or "query_3D_embedding" in var_name \
            or "position_encoder" in var_name or "relative_embedding" in var_name:
            return 0
        elif ".blocks." in var_name and ".residual." not in var_name:
            layer_id = int(var_name[var_name.find(".blocks.") :].split(".")[2]) + 1
...
class CustomLayerDecayOptimizerConstructor(DefaultOptimizerConstructor):
    ...
    def add_params(self, params, module, prefix='', is_dcn_module=None):
        ...
        is_vit = self.paramwise_cfg.get('is_vit', False)
        ...
        for name, param in module.named_parameters():
            ...
            if is_vit:
                layer_id = get_vit_lr_decay_rate(name, num_layers)
            else:
                layer_id = custom_get_num_layer_for_swin(name, num_layers, depths)
