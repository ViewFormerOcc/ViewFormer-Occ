...
class NuScenesExplorer:
    ...
    def render_sample(self,
        ...
        if len(lidar_data) > 0:
            ...
            for (_, sd_token), ax in zip(lidar_data.items(), axes.flatten()[0:]):
        ...
        cam_seq = ['CAM_FRONT_LEFT', 'CAM_FRONT', 'CAM_FRONT_RIGHT',
                                  'CAM_BACK_LEFT', 'CAM_BACK', 'CAM_BACK_RIGHT']
        it_id = 0

        for (_, sd_token), ax in zip(camera_data.items(), axes.flatten()[3:]):

            sd_token = camera_data[cam_seq[it_id]]
            it_id = it_id + 1
        ...
        if out_path is not None:
            ...
            root, name = os.path.split(out_path)
            name = str(self.debug_id).zfill(6) + '_' + name
            new_out_path = root + '/' + name
            plt.savefig(new_out_path)

            self.debug_id = self.debug_id + 1
